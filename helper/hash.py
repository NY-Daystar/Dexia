...
def file_checksum(source: str, content_type: str = "file") -> str:
    ...
    Args:
        source (str): data if `content_type` is str so it's own source,
            if `content_type` is file then source is the path
        content_type (str, optional): type of content to return sha512
            (could be 'file' or 'str'). Defaults to "file".
